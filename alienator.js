...
私は未知の物事や見慣れない物事に対しても柔軟に心を広く保ち、一人一人が違うということを忘れることなく結束していこうと努めます。`,
    ...
    'This alien needs some eyes.\nDrag them onto the body.': segmentJapaneseText({
      string: 'このエイリアンには目が\n必要です！エイリアンの体の上にドラッグして下さい。',
    }),
    'What is your alien name?': 'エイリアンの名前は？',
    'Please tell us two things that alienate you.':
      'あなたを孤立させたりエイリアンにする\n    二つの要素を教えてください。',
    'too long': '長すぎます',
...
Ich werde allem Unbekannten gegenüber aufgeschlossen sein, und darauf zielen unsere Unterschiede zu vereinen.`,
    ...
    'Please tell us two things that alienate you.':
      'Bitte nenne zwei Sachen, die Dich von\n      anderen unterscheiden.',
    'too long': 'zu lang',
    'enter text': 'Text eingeben',
    'Integrating alien into community.\nPlease be patient.':
      'Das Alien wird in die Gemeinschaft integriert.\nBitte habe etwas Geduld.',
  },
}
...
function hashToDNA(opt = {}) {
  ...
  const dna = _.fromPairs(
    _.remove(
      _.forEach(
        _.map(hash.split(separator1), (part) => part.split(separator2)),
        (pair) => {
...
function makeUI(opt = {}) {
  ...
  const previousHeadButton = makeButton({
    group,
    key: 'previousHead',
    frameKey: 'previous',
    onClick: () => alien.showPreviousItem({ type: 'head' }),
  })
  previousHeadButton.position.set(
    bounds.left + randomButton.width / 2 + edgeMargin,
    bounds.height * headYPosFactor + previousNextButtonOffsetY,
  )

  const previousBodyButton = makeButton({
    group,
    key: 'previousBody',
    frameKey: 'previous',
    onClick: () => alien.showPreviousItem({ type: 'body' }),
  })
  previousBodyButton.position.set(
    bounds.left + randomButton.width / 2 + edgeMargin,
    bounds.height * bodyYPosFactor + previousNextButtonOffsetY,
  )

  const nextHeadButton = makeButton({
    group,
    key: 'nextHead',
    frameKey: 'next',
    onClick: () => alien.showNextItem({ type: 'head' }),
  })
  nextHeadButton.position.set(
    bounds.right - randomButton.width / 2 - edgeMargin,
    bounds.height * headYPosFactor + previousNextButtonOffsetY,
  )

  const nextBodyButton = makeButton({
    group,
    key: 'nextBody',
    frameKey: 'next',
    onClick: () => alien.showNextItem({ type: 'body' }),
  })
  nextBodyButton.position.set(
    bounds.right - randomButton.width / 2 - edgeMargin,
    bounds.height * bodyYPosFactor + previousNextButtonOffsetY,
  )
...
function makeButton(opt = {}) {
  ...
  const button = game.add.button(
    ...
    onClick,
    this,
    `ui${capitalizedFrameKey}Over`,
    `ui${capitalizedFrameKey}`,
    `ui${capitalizedFrameKey}Down`,
    `ui${capitalizedFrameKey}`,
    group,
  )
...
function showEditor() {
  ...
  new TimelineMax()
    ...
    .from(
      [ui.nextBodyButton, ui.nextHeadButton],
      { duration: 0.5, x: game.world.centerX + ui.nextHeadButton.width / 2, stagger: 0.1, ease: 'back.out' },
      0.65,
    )
    .from(
      [ui.previousBodyButton, ui.previousHeadButton],
      { duration: 0.5, x: game.world.centerX - ui.previousHeadButton.width / 2, stagger: 0.1, ease: 'back.out' },
      0.65,
    )
    ...
    .from(
      [ui.eyes[5], ui.eyes[4], ui.eyes[3]],
      { duration: 0.5, x: game.world.centerX + ui.eyes[3].eyeball.width / 2, stagger: 0.1, ease: 'back.out(4)' },
      0.75,
    )
    .from(
      [ui.eyes[0], ui.eyes[1], ui.eyes[2]],
      { duration: 0.5, x: game.world.centerX - ui.eyes[2].eyeball.width / 2, stagger: 0.1, ease: 'back.out(4)' },
      0.75,
    )
}
...
function makeTraits(opt = {}) {
  ...
  textField1.warning = makeTextWarning({
    parent: group,
    x: textMargin + textWidth,
    y: textMargin,
    defaultText: localize('too long'),
  })

  game.add.text(
    textMargin,
    textMargin + 210,
    localize('Please tell us two things that alienate you.'),
    textStyle,
    group,
  )
  ...
  textField2.warning = makeTextWarning({
    parent: group,
    x: textMargin + textWidth,
    y: textMargin + 290,
    defaultText: localize('too long'),
  ...
  textField3.warning = makeTextWarning({
    parent: group,
    x: textMargin + textWidth,
    y: textMargin + 555,
    defaultText: localize('too long'),
...
function showTraits() {
  ...
  ui.traits.timeline = new TimelineMax({ onComplete: handleComplete, onReverseComplete: handleReverseComplete })
    ...
    .fromTo(
      ui.cancelButton,
      0.5,
      { x: ui.okButton.positions.editor.x, angle: 0 },
      { x: game.world.centerX - 100, angle: -360, alpha: 1, ease: Power1.easeInOut },
      0.2,
    )
...
function save({ images }) {
  ...
  $.ajax({
    ...
    if (responseObj.success) {
      ...
      notice.show({
        text: 'Oops. We encountered an error.\nThe alien could not be saved.\nPlease reload the page\nto try again.',
        y: alien.y + 150,
        persistent: true,
      })
    }
...
function makeColorSelector(opt = {}) {
  ...
  function getColor(pointer) {
    ...
    if (
      ...
      x <= sprite.x - sprite.anchor.x * sprite.width + sprite.width &&
      y >= sprite.y - sprite.anchor.y * sprite.height &&
      y <= sprite.y - sprite.anchor.y * sprite.height + sprite.height
    ) {
